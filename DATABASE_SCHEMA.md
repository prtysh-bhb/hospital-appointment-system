...
|-------|---------------|--------------|------------|
| users | 10,110 | 500 bytes | ~5 MB |
| doctor_profiles | 100 | 300 bytes | ~30 KB |
| patient_profiles | 10,000 | 400 bytes | ~4 MB |
| specialties | 20 | 200 bytes | ~4 KB |
| appointments | 12,000 | 600 bytes | ~7 MB |
| doctor_schedules | 700 | 200 bytes | ~140 KB |
| prescriptions | 8,000 | 1 KB | ~8 MB |
| notifications | 50,000 | 400 bytes | ~20 MB |
| audit_logs | 100,000 | 500 bytes | ~50 MB |

**Total Estimated Storage**: ~95 MB (data only, excluding indexes)

**With Indexes**: ~200-250 MB
...
### Phase 4: Supporting Tables (Week 4)
9. notifications
10. audit_logs
11. settings
12. password_reset_tokens
...
### Data Retention
- **appointments**: Retain for 7 years (medical records requirement)
- **audit_logs**: Retain for 3 years minimum
- **soft_deletes**: Implement for users and appointments
...
### Monthly Backups
- Archive backup on 1st of month
- Retain for 5 years (compliance)
...
This schema is designed to support a comprehensive hospital appointment management system with:
- ✅ Multi-role user management (admin, doctor, frontdesk, patient)
- ✅ Flexible doctor scheduling with exceptions
...
The schema provides a solid foundation for the two developers to implement the complete system independently following the parallel development plan.
